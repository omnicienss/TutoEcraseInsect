...
bool HelloWorld::init()
{
    ...
    //m_Score;
    __String *myText = __String::createWithFormat("%d", m_Score);
    ...
    //m_NbLife;
    __String *myTextLife = __String::createWithFormat("%d", m_NbLife);
...
*/
    ...
    this->schedule(schedule_selector(HelloWorld::setInsect), 0.25f); //1/30
...
void HelloWorld::setInsect(float pTime)
{
    ...
    //Test si on est en situation de GAMEOVER
    for(Sprite* sprite : m_ListeInsecte) {
        if (sprite->getPositionY() < m_TraitSeparationY) {
            ...
            if(m_NbLife>0)
            {
                m_NbLife--;
                //Affichage des vies
                __String *myText = __String::createWithFormat("%d", m_NbLife);
    ...
    // add "HelloWorld" splash screen"
    cocos2d::Sprite* sprite = Sprite::create("insect01.png");
